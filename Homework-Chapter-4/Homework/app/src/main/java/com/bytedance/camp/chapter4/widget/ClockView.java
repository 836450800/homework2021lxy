...
public class ClockView extends View {
    ...
    private void drawUnit(Canvas canvas) {
        for (int i = 0; i < unitLinePositions.size(); i++) {
            ...
            canvas.drawPoint(linePosition.left, linePosition.top, unitPaint);
        }
    }
    ...
    private void drawTimeNeedles(Canvas canvas) {
        ...
        double radiansSecond=Math.toRadians(second*6-90),radiansMunute=Math.toRadians(minute*6+second*0.1-90);
        double radiansHour=Math.toRadians(hour*30+minute*0.5+second*0.5/60.0-90);
        StartX=centerX;StartY=centerY;
        EndX=(float)(StartX+radius*SECOND_NEEDLE_LENGTH_RATIO*Math.cos(radiansSecond));
        EndY=(float)(StartY+radius*SECOND_NEEDLE_LENGTH_RATIO*Math.sin(radiansSecond));
        needlePaint.setStrokeWidth(SECOND_NEEDLE_WIDTH);
        canvas.drawLine(StartX,StartY,EndX,EndY, needlePaint);
        StartX=centerX;StartY=centerY;
        EndX=(float)(StartX+radius*MINUTE_NEEDLE_LENGTH_RATIO*Math.cos(radiansMunute));
        EndY=(float)(StartY+radius*MINUTE_NEEDLE_LENGTH_RATIO*Math.sin(radiansMunute));
        needlePaint.setStrokeWidth(MINUTE_NEEDLE_WIDTH);
        canvas.drawLine(StartX,StartY,EndX,EndY, needlePaint);
        EndX=(float)(StartX+radius*HOUR_NEEDLE_LENGTH_RATIO*Math.cos(radiansHour));
        EndY=(float)(StartY+radius*HOUR_NEEDLE_LENGTH_RATIO*Math.sin(radiansHour));
